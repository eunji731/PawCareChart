  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-3 gap-3 lg:gap-4">
      ...
      {isMedical && record.medicationStatus !== 'NONE' && (
        <div className={`col-span-1 md:col-span-3 rounded-[20px] md:rounded-[24px] lg:rounded-[32px] p-5 lg:p-6 flex flex-row items-center justify-between gap-3 md:gap-5 border shadow-sm
          ${record.medicationStatus === 'ACTIVE' ? 'bg-[#FF6B00]/5 border-[#FF6B00]/20' : 'bg-green-50/50 border-green-200/50'}
        `}>
          <div className="flex items-center gap-3 md:gap-4 md:pl-3">
            ...
            <div className="flex flex-col gap-0.5 md:gap-1">
               <span className="text-[10px] md:text-[11px] font-black text-stone-500 uppercase tracking-widest">Medication Status</span>
               <span className="text-[13px] md:text-[15px] font-black text-[#2D2D2D]">약을 복용 중인가요?</span>
            </div>
          </div>
          <span className={`px-4 py-2.5 md:px-5 md:py-3.5 rounded-[12px] md:rounded-[16px] text-[11px] md:text-[13px] font-black tracking-widest uppercase text-center shrink-0
